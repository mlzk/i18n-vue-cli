...
function readFileList(dir, src, pages, langObj, ignoredir, fileKey) {
    let filesList = []
    let ps = []
                    ...
                    rl.on('line', (line) => {
                        ...
                        while (str) {
                            ...
                            if (['.vue'].includes(extname)) {
                                ...
                                if (isTemplate) {
                                    ...
                                    props.forEach(item => {
                                        ...
                                        if (item.indexOf(`="${str}"`) > -1 || item.indexOf(`='${str}'`) > -1) {
                                            let LangItem = ':' + item.replace(str, `$tt('${langObj[str]}','${fileKey}')`)
                                ...
                                // 过滤之前匹配的过滤的行内翻译
                                if (ContentLine.includes(`="$tt`) || ContentLine.includes(`='$tt`)||ContentLine.includes(`$tt(`) ) {

                                } else {
                                    if (ContentLine.includes(`"${str}"`) ||
                                        ContentLine.includes(`'${str}'`)) {
                                        ContentLine = ContentLine.replace(`'${str}'`, isTemplate ? `$tt('${langObj[str]}','${fileKey}')` : `$tt('${langObj[str]}','${fileKey}')`)
                                        ContentLine = ContentLine.replace(`"${str}"`, isTemplate ? `"{{$tt('${langObj[str]}','${fileKey}')}}"` : `$tt('${langObj[str]}','${fileKey}')`)
                                    } else {
                                        ContentLine = ContentLine.replace(`${str}`, isTemplate ? `{{$tt('${langObj[str]}','${fileKey}')}}` : `$tt('${langObj[str]}','${fileKey}')`)
                                    }
                            ...
                            if (['.js'].includes(extname)) {
                                ContentLine = ContentLine.replace(`'${str}'`, `$tt('${langObj[str]}','${fileKey}')`)
                                ContentLine = ContentLine.replace(`"${str}"`, `$tt('${langObj[str]}','${fileKey}')`)
                            }
                            hasReplaced = true
...
module.exports.writeLang = (src = 'srcDist', pages = 'pages', LangJson = '/zh_cn.json', ignoredir, fileKey) => {
    ...
    readFileList(filePath, src, pages, langObj, ignoredir, fileKey).then(() => {
