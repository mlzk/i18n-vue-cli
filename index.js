...
program.command('getlang [src]')
    ...
    .option('-e, --fileKey <fileKey>', '[optional]额外字段', value => {
        return value
    })
    .action((src = 'src', {dir = ['pages', 'components'], filename = 'zh_cn.json', ignoredir,fileKey}) => {
        console.log(fileKey)
        if(filename.includes('.json')) {
            getLang.getLang(src, dir, filename, ignoredir,fileKey)
        } else {
...
program.command('writelang [src]')
    ...
    .action((src = 'srcDist', {dir = ['pages', 'components'], filename = 'zh_cn.json', ignoredir,fileKey}) => {
        if(filename.includes('.json')) {
            writeLang.writeLang(src, dir, filename, ignoredir,fileKey)
        } else {
            console.error('filename 必须是json文件类型')
        }
    })
